...
let mouseDrag   = false;
//
let objects = [/*'Tetra','Cube','Cylinder',*/'Sphere','Torus','Thing','Thing2','Cool'];
let showWhich = 1;
let smoothness = 32;
let layers = 20;
let gradient = true; // if this isn't true, then it will use the basic blue/white color scheme
let geodesic = true; // for the sphere and torus, this adds a geodesic around the equator - it would be cool to make it go around anywhere else as well
//
...
function makeRevolution(name, points, Geo = [], polarS = false) {
...
function addGeodesic(radius, distance=0, x1=radius+distance, y1=0, z1=0, theta,) { // trying to figure out how to add a gedesic to these 3d shapes, this might be able to be done with the drawSweep eventually
    glColor3f(1,0,0);
...
function makeSphere() { // Makes a sphere
    const radius = 1;
    let points = [];
    for (let i = 0; i <= layers; i++) {
        points.push({x: radius * Math.sin(Math.PI / layers * i), y: radius * -1 * Math.cos(-1 * Math.PI / layers * i)})
    }
    let Geo = [];
    if (geodesic) {
        Geo = addGeodesic(radius);
    }
    makeRevolution('Sphere', points, Geo); // depends on makeRevolution
}

function makeTorus() { // Makes a torus - aligns color interestingly if layers is odd and gradient = false (prioritizes color difference between adjacent squares over rectangularity)
    const size = 0.8; // does weird things if size < width (disco ball effect if equal to 0) - becomes a twice covered sphere
    const width = 0.5;
    let points = [];
    ...
    let Geo = [];
    if (geodesic) {
        Geo = addGeodesic(width, size);
    }
    makeRevolution('Torus',points, Geo, true); // depends on makeRevolution
}

function makeThing() { // Makes a cool pillar thing, like the center section of a torus
    const width = 1.3;
    let points = [];
    for (let i = 0; i <= layers; i++) {
        points.push({x: width - Math.sin(Math.PI / layers * i), y: -1 * Math.cos(-1 * Math.PI / layers * i)})
    }
    makeRevolution('Thing',points); // (depends on makeRevolution)
}

function makeThing2() { // Makes a closed-at-the-ends rotation of an equation of the form y=sin(rx^2) + t around the y-axis
    let points = [];
    const midWidth = 1; // t
    const squish = 6.8; // r
    const totalH = 2; // a - this is the max height displayed (from -a/2 to a/2)
    for (let i = 0; i <= layers; i++) {
    ...
    makeRevolution('Thing2',points); // (depends on makeRevolution)
}

function makeCool() { // This is the cool christmas tree sphere (does not depend on makeRevolution)
...
function drawObject() { // Draw the object selected by the user
    glBeginEnd(objects[Math.min(showWhich -1,objects.length - 1)])
}

function drawScene() { // Issue GL calls to draw the scene
